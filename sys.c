...
SYSCALL_DEFINE1(my_xtime, struct timespec __user *, get_Time)
{
	...
	if(!access_ok(VERIFY_WRITE, get_Time, sizeof(struct timespec)))
	{
		return -(EFAULT);
	}

	get_Time->tv_sec = curr_Time.tv_sec;
	get_Time->tv_nsec = curr_Time.tv_nsec;

	printk(KERN_INFO "My Time: %ld", get_Time->tv_nsec);
